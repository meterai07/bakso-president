...
<div class="h-max bg-primary-50">
  ...
  <div class="flex items-center justify-center">
    <div class="flex flex-col bg-cover items-center justify-center w-[500px] h-fit py-12 gap-4 shadow-lg rounded-[50px]" style="background-image: url(/assets/img/login-bg.svg)">
      <h1 class="text-3xl font-extrabold text-neutral-900 font-lora">Edit Menu</h1>
      <form action="/admin/menu/update/{{ $menu->id }}" method="post" enctype="multipart/form-data" class="w-full max-w-sm mx-auto">
        ...
        <div class="mb-4">
          ...
          <input type="text" name="name" id="name" placeholder="Nama Menu" value="{{ $menu->name }}" class="px-4 py-2 border border-neutral-200 rounded-lg shadow-md w-full focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-transparent focus:duration-500">
        </div>
        <div class="flex flex-row mb-4 gap-2 items-center">
          @foreach($categories as $category)
          <label class="flex-row items-center text-gray-700 text-sm font-bold">
            <input type="radio" name="category_id" id="category_id" value="{{ $category->id }}" class="" {{ $menu->category_id == $category->id ? 'checked' : '' }}>
            {{ $category->name }}
          </label>
          @endforeach
        </div>
        <div class="mb-4">
          ...
          <input type="text" name="description" id="description" placeholder="Deskripsi Menu" value="{{ $menu->description }}" class="px-4 py-2 border border-neutral-200 rounded-lg shadow-md w-full focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-transparent focus:duration-500">
        </div>
        <div class="mb-4">
          <label for="price" class="block text-gray-700 text-sm font-bold mb-2">Harga Menu</label>
          <input type="number" name="price" id="price" placeholder="Harga Menu" value="{{ $menu->price }}" class="px-4 py-2 border border-neutral-200 rounded-lg shadow-md w-full focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-transparent focus:duration-500">
        </div>
        <div class="flex flex-col mb-4 justify-center">
          <img src="{{ asset('storage/' . $menu->image) }}" alt="{{ $menu->name }}" class="flex mb-2 ml-auto mr-auto" width="200">
