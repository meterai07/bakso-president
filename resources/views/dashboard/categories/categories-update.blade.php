@extends('dashboard.layouts.main', ['page' => 'categories-update'])
...
<div class="h-screen bg-primary-50">
    ...
    <div class="flex items-center justify-center mt-auto mb-auto">
        ...
        <h1 class="text-3xl font-extrabold text-neutral-900 font-lora">Edit Kategori</h1>
        <form action="/admin/categories/update/{{ $category->id }}" method="post" enctype="multipart/form-data" class="w-full max-w-sm mx-auto">
            @csrf
            <div class="mb-4">
            <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Nama Kategori</label>
            <input type="text" name="name" id="name" placeholder="Nama Category" value={{ $category->name }} class="px-4 py-2 border border-neutral-200 rounded-lg shadow-md w-full focus:outline-none focus:ring-2 focus:ring-gray-200 focus:border-transparent focus:duration-500">
            </div>
